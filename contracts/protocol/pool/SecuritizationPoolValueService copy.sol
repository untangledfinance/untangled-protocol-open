...
//         uint256 riskScoresLength = pool.getRiskScoresLength();
...
//             uint32 daysPastDue = getDaysPastDueByIdx(poolAddress, riskScoreIdx);
//             if (overdue < daysPastDue) return (false, 0);
//             else if (riskScoreIdx == riskScoresLength - 1) {
//                 return (true, riskScoreIdx);
//             } else {
//                 uint32 nextDaysPastDue = getDaysPastDueByIdx(poolAddress, riskScoreIdx + 1);
//                 if (overdue < nextDaysPastDue) return (true, riskScoreIdx);
//             }
//         }
//     }

//     function getDaysPastDueByIdx(address securitizationPool, uint256 idx) private view returns (uint32) {
//         (uint32 daysPastDue, , , , , , , , , , ) = ISecuritizationPool(securitizationPool).riskScores(idx);
//         return daysPastDue;

//         // RiskScore memory riskScore = getRiskScoreByIdx(securitizationPool, idx);
//         // return riskScore.daysPastDue;
//     }
...
//             uint32 writeOffAfterCollectionPeriod // daysPastDue: _daysPastDues[i],
