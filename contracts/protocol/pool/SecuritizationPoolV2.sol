...
import {ContractError} from '../../libraries/ContractError.sol';
 ...
 */
contract SecuritizationPoolV2 is ISecuritizationPool, IERC721ReceiverUpgradeable {
    ...
    function initialize(
        ...
        if (_minFirstLossCushion >= 100 * RATE_SCALING_FACTOR)
            revert ContractError.SecuritizationPoolMinFirstLostGreaterThan100();
        if (_currency == address(0)) revert ContractError.SecuritizationPoolInvalidCurrency();
        ...
        pot = address(this);
        if (!IERC20(_currency).approve(pot, type(uint256).max))
            revert ContractError.SecuritizationPoolCurrencyApprovalFailed();
    ...
    function exportAssets(
        ...
        for (uint256 i = 0; i < tokenIdsLength; i = UntangledMath.uncheckedInc(i)) {
            if (!_removeNFTAsset(tokenAddress, tokenIds[i])) {
                revert ContractError.SecuritizationPoolAssetNotExisted();
            }
        }
    ...
    function withdrawAssets(
        ...
        for (uint256 i = 0; i < tokenIdsLength; i = UntangledMath.uncheckedInc(i)) {
            if (!_removeNFTAsset(tokenAddresses[i], tokenIds[i])) {
                revert ContractError.SecuritizationPoolAssetNotExisted();
            }
        }
        for (uint256 i = 0; i < tokenIdsLength; i = UntangledMath.uncheckedInc(i)) {
