...
describe('MinFirstLoss', () => {
  ...
  // Wallets
  let untangledAdminSigner, poolCreatorSigner, poolACreator, originatorSigner, lenderSigner, secondLenderSigner,
    relayer;
  ...
  before('create fixture', async () => {
    // Init wallets
    [untangledAdminSigner, poolCreatorSigner, poolACreator, originatorSigner, lenderSigner, secondLenderSigner, relayer] = await ethers.getSigners();
  ...
  describe('Check min first loss when buying tokens', () => {
    it('should revert if try to buy SOT when total JOT supply is 0 (zero JOT was sold)', async () => {
      ...
      await expect(
        securitizationManagerContract.connect(lenderSigner).buyTokens(mintedIncreasingInterestTGEContract.address, stableCoinAmountToBuySOT),
    ...
    it('should revert if try to buy SOT with amount violates min first loss', async () => {
      ...
      await expect(
        securitizationManagerContract.connect(lenderSigner).buyTokens(mintedIncreasingInterestTGEContract.address, amountToBuySOT),
      ).to.be.revertedWith('MinFirstLoss is not satisfied');
    });
