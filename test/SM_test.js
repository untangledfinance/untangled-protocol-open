...
contract('SecuritizationManager', (accounts) => {
    ...
    it(' Should buy token successfully ', async () => {
        ...
        await noteToken.grantRole(MINTER_ROLE, tgeAddress);
        await noteToken.grantRole(MINTER_ROLE,  mintedIncreasingInterestTGE.address);
        await noteToken.grantRole(MINTER_ROLE,  smAddress);
        console.log(193, mintedIncreasingInterestTGE.address)
        let mintTGEAddress = await registry.getAddress(17);
        console.log(199, mintTGEAddress);
        let txbuy = await mintedIncreasingInterestTGE.buyTokens(accounts[0],accounts[0], "100000000000000000");
        console.log(201, txbuy)
